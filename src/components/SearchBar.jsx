...
export function SearchBar({ initialValue = "" }) {
    ...
    return (
        <div className="relative">
            <form onSubmit={submit} className="flex w-full flex-col gap-2 rounded-xl border border-[#c8dade] bg-white p-2 shadow-sm sm:flex-row">
                <label className="relative flex-1">
                    <Search className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-ink" />
                    <input value={query} onChange={(event) => setQuery(event.target.value)} onFocus={() => setIsFocused(true)} onBlur={() => setTimeout(() => setIsFocused(false), 150)} className="h-12 w-full rounded-lg bg-transparent pl-10 pr-3 text-base text-ink outline-none placeholder:text-muted-ink focus:ring-2 focus:ring-ink/15" placeholder="O que você precisa aprender?" aria-label="Pesquisar tutoriais" autoComplete="off" />
                </label>
